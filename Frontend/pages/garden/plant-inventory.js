...
export default function PlantInventory(props) {
    ...
    // update imagesObj with all the image URLs of plants in the gardne
    useEffect(() => {
    ...
    const addPlantsButton = <Col>
        <TouchableOpacity style={styles.emptyImage} onPress={() => props.navigation.navigate("Plants", {gardenId: props.gardenData.id})}>
            <Image style={styles.addButton} source={require("../../../assets/images/add.png")}></Image>
        </TouchableOpacity>
    </Col>

    // dynamically create a row, placing PlantIcon's if there is a plant at that index, or
    // putting an empty cell otherwise. The cell right after the last PlantIcon is a button to 
    // add plants
